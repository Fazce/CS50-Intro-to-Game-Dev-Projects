...
function PlayState:init()
    ...
    -- Local variable that multiples the amount of points a variant of a block gives
    self.variantMultiplier = 1.5
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                -- Or swap the blocks back if there is no match
                :finish(function()
                    if not self.board:calculateMatches() then
                        -- Revert swap if the swapped blocks have no match
                        -- Works the exact same as the swap code above
                        local revertX = self.highlightedTile.gridX
                        local revertY = self.highlightedTile.gridY
                        self.highlightedTile.gridX = newTile.gridX
                        self.highlightedTile.gridY = newTile.gridY
                        newTile.gridX = revertX
                        newTile.gridY = revertY
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        -- also extends the time by a second per block in the match
        for k, match in pairs(matches) do
            for j, tile in pairs(match) do
                bonusPoints = self.variantMultiplier ^ (tile.variety - 1)
            end
            self.score = self.score + (#match * 50 * bonusPoints)
