...
function Brick:hit()
    ...
    -- In the scenario that a locked brick is hit without the key powerup
...
function Brick:render()
    if self.inPlay then
        -- multiply color by 4 (-1) to get our color offset, then add tier to that
         ...
         -- Since there is 21 bricks total, locked brick is 22
        local index = self.lockedBrick and 22 or (1 + ((self.color - 1) * 4) + self.tier)
        love.graphics.draw(gTextures['main'], 
            gFrames['bricks'][index], self.x, self.y)
    end
end
